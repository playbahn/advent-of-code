        ...
        if ins_len == 3 {       // SIGNAL
            match ins.split_ascii_whitespace().next().unwrap() {
                ...
                wire_sig => {
                    map.insert(
                        wire.to_owned(),
                        Ops(WIRE(wire_sig.to_string()))
                    );
                },
            }
        } else if ins_len == 4 {        // NOT
            let wire_sig: &str = ins.split_ascii_whitespace().skip(1).next().unwrap();
            map.insert(
                wire.to_string(),
                Ops(NOT(wire_sig.to_string())));

        } else {    // [`ins_len`] == 5     // SHIFT, AND, OR
            ...
            if ins.contains("OR") {
                ...
                map.insert(
                    wire.to_owned(),
                    Ops(OR(in1.to_string(), in2.to_string()))
                );
                ...
                map.insert(
                    wire.to_owned(),
                    Ops(AND(in1.to_owned(), in2.to_owned()))
                );
            } else {        // SHIFT
                match vec5[1] {
                    "LSHIFT" => {
